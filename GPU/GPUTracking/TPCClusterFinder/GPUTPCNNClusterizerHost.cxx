...
void GPUTPCNNClusterizerHost::init(const GPUSettingsProcessingNNclusterizer& settings, bool useDeterministicMode)
{
  ...
  mOrtOptions = {
    ...
    {"deterministic-compute", std::to_string(useDeterministicMode ? 1 : settings.nnInferenceUseDeterministicCompute)},
...
void GPUTPCNNClusterizerHost::initClusterizer(const GPUSettingsProcessingNNclusterizer& settings, GPUTPCNNClusterizer& clustererNN, int32_t maxFragmentLen, int32_t maxAllowedTimebin)
{
  ...
  clustererNN.mNnClusterizerElementSize = clustererNN.mNnClusterizerChargeArraySize + (settings.nnClusterizerAddIndexData ? 3 : 0) + (settings.nnClusterizerAddMeanSigma ? 4 : 0);
  ...
  if(settings.nnClusterizerSetNetworkFlags) {
    clustererNN.mNnClusterizerModelFlagNumOutputNodes = mModelFlag.getNumOutputNodes()[0][1];
  }
}
