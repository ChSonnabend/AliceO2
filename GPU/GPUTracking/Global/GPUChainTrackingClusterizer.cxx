      ...
      mRec->runParallelOuterLoop(doGPU, maxLane, [&](uint32_t lane) {
        ...
        if (GetProcessingSettings().nn.applyNNclusterizer) {
          ...
          for (int batch = 0; batch < std::ceil((float)clusterer.mPmemory->counters.nClusters / clustererNN.nnClusterizerBatchedMode); batch++) {
            ...
            if(GetProcessingSettings().nn.setDeconvolutionFlags){
              runKernel<GPUTPCNNClusterizerKernels, GPUTPCNNClusterizerKernels::publishDeconvolutionFlags>({GetGrid(iSize, lane), krnlRunRangeNone}, iSector, clustererNN.nnClusterizerDtype, withMC, batchStart); // Running the NN for regression class 1
            }
