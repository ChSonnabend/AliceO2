  ...
  for(int padrow = 0; padrow < o2::tpc::constants::MAXGLOBALPADROW; padrow++){
    ...
    for(int track_id = 0; track_id < overlap_info_trkid_map[padrow].size(); track_id++){
      if(misc_track_id_info[padrow][track_id][0] > 0){
        ...
        overlap_info[padrow][track_id][3] = misc_track_id_info[padrow][track_id][0];
        overlap_info[padrow][track_id][4] = misc_track_id_info[padrow][track_id][1];
      }
    }
  }
}
...
void qaCluster::runQa(int sector)
{
  ...
  if (mode.find(std::string("native")) != std::string::npos && create_output == 1) {
    ...
    float sec = sector, nat_row = 0, nat_time = 0, nat_pad = 0, nat_sigma_time = 0, nat_sigma_pad = 0,  nat_qTot = 0, nat_qMax = 0, id_sigma_pad = 0, id_sigma_time = 0, id_row = 0, id_time = 0, id_pad = 0, id_qTot = 0, id_qMax = 0, area_overlap = 0, charge_overlap = 0, tot_area = 0, tot_charge = 0;;
...
DataProcessorSpec processIdealClusterizer(ConfigContext const& cfgc, std::vector<InputSpec>& inputs, std::vector<OutputSpec>& outputs)
{
  ...
  return DataProcessorSpec{
    ...
    Options{
      ...
      {"network-use-float-label", VariantType::Int, 0, {"If 1, transform network classification output as NN_out = 1.f/(1.f + std::exp(-NN_out));"}},
